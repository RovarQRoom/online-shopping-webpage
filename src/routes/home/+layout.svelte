<script lang="ts">
	import '../../app.postcss';
	import { Navbar, NavBrand, NavLi, NavUl, NavHamburger, Button, Input, Search, DarkMode, Dropdown, DropdownItem, DropdownDivider, Chevron, Checkbox, Helper, Drawer, CloseButton, Table, TableHead, TableHeadCell, TableBody, TableBodyRow, TableBodyCell, Avatar } from 'flowbite-svelte';
  import { CartOutline, CartPlusAltSolid, OrdoredListOutline, UserCircleSolid } from 'flowbite-svelte-icons';
	import { kubakLogo } from '../../components/Images/kubak.logo';
	import { onMount } from 'svelte';
	import { categoryHandlers, itemsHandlers, userWritable } from '$lib/store/firebase-store';
...
cart.subscribe((value) => {
  ...
  async function buyingItems(){
    if($userWritable.user){
      await httpsCallable(functions, 'buyItems')({items: $cart, client_uid: $userWritable.user.uid, total_amount: overhaulPrice, address: $userWritable.user.displayName});
    }else{
      goto('/login');
    }
  }
...
<main class="bg-slate-100 dark:bg-gray-950 w-full h-full">
  <Drawer placement='right' transitionType="fly" transitionParams={transitionParamsRight} bind:hidden={hidden6} id='sidebar6' width="w-full">
    <div class='flex items-center'>
      <h5
        id="drawer-label"
        class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400">
        <svg class="w-5 h-5 mr-2" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>Cart
      </h5>
      <CloseButton on:click={() => (hidden6 = true)} class='mb-4 dark:text-white'/>
        ...
        <Table noborder={true}>
          ...
          <TableBody>
            {#each $cart as item}
            <TableBodyRow>
              <TableBodyCell><Avatar class="object-contain p-1" src={item.item_image} rounded></Avatar></TableBodyCell>
    ...
    <Navbar let:hidden let:toggle class="fixed z-30 top-0 backdrop-blur-lg bg-[#ffffffb3] dark:bg-[#212121b3]">
        <NavBrand href="/" class="w-full flex justify-center items-center">
          <img
            ...
            alt="Kubak"
          />
          <span class="self-center whitespace-nowrap text-md md:text-lg font-semibold dark:text-white uppercase">
